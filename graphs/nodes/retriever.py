...
def retriever_node(state):
    ...
    matched_muscle = next((m for m in all_muscle_names if m in workout_request), None)
    ...
    # A. LOWER BODY logic
    elif "lower" in workout_request:
        patterns = ["squat", "hinge", "extension"]
        for p in patterns:
            matches = [ex['name'] for ex in safe_pool if ex.get('pattern') == p and ex.get('type') == 'strength']
            if matches:
                random.shuffle(matches)
                menu[p.upper()] = matches[:3]                


    # B. UPPER BODY logic
    elif "upper" in workout_request:
    ...
    # C. CARDIO logic
    elif "cardio" in workout_request:
    ...
    # D. MOBILITY logic
    elif "mobility" in workout_request:
